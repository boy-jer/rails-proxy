require 'test_helper'

class ProxyControllerTest < ActionController::TestCase
  def setup
    stub_http_request(:post, "https://railsbp.com/sync_proxy")
  end

  def test_hook_without_token
    post :hook, :payload => payload_body
    assert_equal "not authenticate", response.body
  end

  def test_hook_with_wrong_token
    post :hook, :token => "9876543210", :payload => payload_body
    assert_equal "not authenticate", response.body
  end

  def test_hook_with_not_match_branch
    post :hook, :token => "1234567890", :payload => payload_body
    assert_equal "skip", response.body
  end

  def test_hook_success
    Git::Base.any_instance.expects(:reset_hard).with("473d12b3ca40a38f12620e31725922a9d88b5386")
    post :hook, :token => "1234567890", :payload => payload_body.sub("develop", "master")
    assert_equal "success", response.body
  end

  def test_hook_failure
    Git.expects(:clone).raises(Exception, "test")
    post :hook, :token => "1234567890", :payload => payload_body.sub("develop", "master")
    assert_equal "failure", response.body
  end

  def test_configs
    post :configs, :configs => configs_body
    assert_equal configs_body, File.read(RAILSBP_CONFIG["build_path"] + "/config/rails_best_practices.yml")
  end

  def payload_body
    body = <<-EOF
    {
      "before": "a6ab010bc21151e238c73d5229c36892d51c2d4f",
      "repository": {
        "url": "https://github.com/railsbp/rails-bestpractices.com",
        "name": "rails-bestpractice.com",
        "description": "rails-bestpractices.com",
        "watchers": 64,
        "forks": 14,
        "private": 0,
        "owner": {
          "email": "flyerhzm@gmail.com",
          "name": "Richard Huang"
        }
      },
      "commits": [
        {
          "id": "af9718a9bee64b9bbbefc4c9cf54c4cc102333a8",
          "url": "https://github.com/railsbp/rails-bestpractices.com/commit/af9718a9bee64b9bbbefc4c9cf54c4cc102333a8",
          "author": {
            "email": "flyerhzm@gmail.com",
            "name": "Richard Huang"
          },
          "message": "fix typo in .travis.yml",
          "timestamp": "2011-12-25T18:57:17+08:00",
          "modified": [".travis.yml"]
        },
        {
          "id": "473d12b3ca40a38f12620e31725922a9d88b5386",
          "url": "https://github.com/railsbp/rails-bestpractices.com/commit/473d12b3ca40a38f12620e31725922a9d88b5386",
          "author": {
            "email": "flyerhzm@gmail.com",
            "name": "Richard Huang"
          },
          "message": "copy config yaml files for travis",
          "timestamp": "2011-12-25T20:36:34+08:00"
        }
      ],
      "after": "473d12b3ca40a38f12620e31725922a9d88b5386",
      "ref": "refs/heads/develop"
    }
    EOF
  end

  def configs_body
    "--- \nMoveFinderToNamedScopeCheck: {}\n\nUseModelAssociationCheck: {}\n\nUseScopeAccessCheck: {}\n\nAddModelVirtualAttributeCheck: {}\n\nReplaceComplexCreationWithFactoryMethodCheck: \n  attribute_assignment_count: 2\nMoveModelLogicIntoModelCheck: \n  use_count: 4\nOveruseRouteCustomizationsCheck: \n  customize_count: 3\nNeedlessDeepNestingCheck: \n  nested_count: 2\nNotUseDefaultRouteCheck: {}\n\nKeepFindersOnTheirOwnModelCheck: {}\n\nLawOfDemeterCheck: {}\n\nUseObserverCheck: {}\n\nIsolateSeedDataCheck: {}\n\nAlwaysAddDbIndexCheck: {}\n\nUseBeforeFilterCheck: \n  customize_count: 2\nMoveCodeIntoControllerCheck: {}\n\nMoveCodeIntoModelCheck: \n  use_count: 2\nMoveCodeIntoHelperCheck: \n  array_count: 3\nReplaceInstanceVariableWithLocalVariableCheck: {}\n\nDryBundlerInCapistranoCheck: {}\n\nUseSayWithTimeInMigrationsCheck: {}\n\nUseQueryAttributeCheck: {}\n\nRemoveTrailingWhitespaceCheck: {}\n\nUseMultipartAlternativeAsContentTypeOfEmailCheck: {}\n\nSimplifyRenderInViewsCheck: {}\n\nSimplifyRenderInControllersCheck: {}\n\nRemoveEmptyHelpersCheck: {}\n\nRemoveTabCheck: {}\n\nRestrictAutoGeneratedRoutesCheck: {}\n\nRemoveUnusedMethodsInModelsCheck: \n  except_methods: []\n\nRemoveUnusedMethodsInControllersCheck: \n  except_methods: []\n\nRemoveUnusedMethodsInHelpersCheck: \n  except_methods: []\n\n"
  end
end
